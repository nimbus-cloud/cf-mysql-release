...
describe 'Enforcing MySQL quota', :service => true do
  ...
  it 'enforces the storage quota' do
    ...
    BlueShell::Runner.run "gcf login -a #{api_endpoint} -u #{username} -p #{password} -o #{org} -s #{space}"
    BlueShell::Runner.run "gcf cs #{service_name} #{plan_name} #{instance_name}"
    BlueShell::Runner.run "gcf push"
  end
